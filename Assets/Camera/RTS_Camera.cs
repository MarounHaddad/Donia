...
namespace RTS_Cam
{
    ...
    public class RTS_Camera : MonoBehaviour
    {
		...
		public float MinCamerZoom;
		public float MaxCamerZoom;
        ...
        private void Start()
        {
			...
			GetComponent<Camera> ().orthographicSize = DefaultCameraZoom;
        }
        ...
        private void Move()
        {
            ...
            if (useScreenEdgeInput)
            {
                ...
                m_Transform.Translate(desiredMove, Space.Self);
				//Debug.Log (m_Transform.position.x + " " + m_Transform.position.y + " " + m_Transform.position.z);
            }       
		...
		private void OrthographicZoom(){
			try {
				if (ScrollWheel==0)
						return ;
				if(GetComponent<Camera>().Equals(null)) return;
				float t = GetComponent<Camera>().orthographicSize;
				if ( GetComponent<Camera>().orthographicSize<= MaxCamerZoom && GetComponent<Camera>().orthographicSize>= MinCamerZoom )
					GetComponent<Camera>().orthographicSize -= ScrollWheel * Time.deltaTime * scrollWheelZoomingSensitivity;
				else
					if (GetComponent<Camera>().orthographicSize >=MaxCamerZoom)
						GetComponent<Camera>().orthographicSize =MaxCamerZoom;
					else
						if (GetComponent<Camera>().orthographicSize <=MinCamerZoom)
							GetComponent<Camera>().orthographicSize =MinCamerZoom;
		...
		private void FollowMinimap()
		{
			if (Input.GetMouseButtonDown(0)){ // if left button pressed...
				Vector3[] corners = new Vector3[4];
				minimapImage.rectTransform.GetWorldCorners(corners);
				Rect newRect = new Rect(corners[0], corners[2]-corners[0]);
				float posX, posY;
				posX = Mathf.Abs(Input.mousePosition.x -newRect.position.x   );
				posY =  Mathf.Abs(Input.mousePosition.y - newRect.position.y  )  ;
				if (!newRect.Contains (Input.mousePosition))
					return;
				//Debug.Log("X " + posX +" Y" + posY);
				float posCameraPosX, posCameraPosY;
				posCameraPosX = ( ((posY)) * MapTerrain.terrainData.size.z / 225)-50;
				posCameraPosY = (((255-posX)) * MapTerrain.terrainData.size.x / 225)-50 ;   
				//Debug.Log("X " + posX + " Cam " + posCameraPosX+  " Y " + posY + " Cam " + posCameraPosY);
				Vector3 desiredMove = new Vector3(posCameraPosX, 30 , posCameraPosY);
